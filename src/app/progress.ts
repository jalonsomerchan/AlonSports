    ...
    <section class="page progress-page">
      ...
      @else {
        ...
        <section class="readiness-card" [class.warning]="statusTone() === 'warning'" [class.fresh]="statusTone() === 'fresh'">
          ...
          <div class="readiness-score"><strong>{{ signed(current()?.form) }}</strong><span>forma</span></div>
        </section>

        <div class="load-grid">
          <article class="detail-card"><span>CARGA · 7 DÍAS</span><strong>{{ number(acuteScore()) }}</strong><p>Esfuerzo reciente</p></article>
          <article class="detail-card"><span>FITNESS</span><strong>{{ number(current()?.fitness) }}</strong><p>Base de las últimas semanas</p></article>
          <article class="detail-card"><span>FATIGA</span><strong>{{ number(current()?.fatigue) }}</strong><p>Respuesta a la carga</p></article>
          <article class="detail-card"><span>DESCANSO</span><strong>{{ restDays() }} <small>días</small></strong><p>Sin actividad en 7 días</p></article>
        </div>
        ...
        <article class="calendar-panel"><div class="section-heading compact"><div><p class="eyebrow">CALENDARIO · 14 DÍAS</p><h2>Actividad y continuidad</h2></div><span class="calendar-summary">{{ activeDays() }} días activos</span></div><div class="calendar-strip">@for (day of calendarDays(); track day.key) { <div class="calendar-day" [class.today]="day.today" [class.active]="day.active" [class.planned]="day.planned"><span>{{ day.label }}</span><strong>{{ day.number }}</strong><i>{{ day.active ? '●' : day.planned ? '○' : '·' }}</i></div> }</div></article>
      }
    </section>
  `,
  styles: [`
...
export class ProgressPage {
  ...
  private goalValue(goal: Goal) { const start = goal.period_start ?? ''; const end = goal.period_end ?? ''; const daily = ((this.stats()?.['daily_30'] ?? []) as CalendarActivity[]).filter(item => { const day = this.dayKey(item.start_date_local); return day >= start && day <= end; }); const fromDaily = goal.sport_type ? 0 : daily.reduce((sum, item) => sum + this.metricValue(goal.metric, item), 0); const fromCalendar = this.activities().filter(item => { const day = this.dayKey(item.start_date_local); return day >= start && day <= end && (!goal.sport_type || goal.sport_type === item.sport_type); }).reduce((sum, item) => sum + this.metricValue(goal.metric, item), 0); return fromDaily > 0 ? fromDaily : fromCalendar; }
